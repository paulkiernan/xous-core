...
const FONT5X7: [[u8; 7]; 32] = [
    ...
    [0b00000, 0b10000, 0b10000, 0b10000, 0b10000, 0b10000, 0b01110],
    ...
    [0b00000, 0b00100, 0b00000, 0b00100, 0b00100, 0b00100, 0b01110],
...
fn glyph_index(c: char) -> usize {
    match c {
        'A' => 0, 'B' => 1, 'D' => 2, 'E' => 3, 'I' => 4, 'L' => 5, 'N' => 6, 'P' => 7,
...
pub fn render(field: &mut [u32; MAX_WORDS], _w: isize, _h: isize) {
    ...
    render_text5(field, "zero@paulynomial.com", 9, 64, 5, 180);
    render_text5(field, "PRESENTS", 40, 82, 6, 210);

    // footer rule + shaded strip
    divider(field, 98);
    checker_strip(field, 106);
}
